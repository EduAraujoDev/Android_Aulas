...
import com.eduaraujodev.torcedometro.model.Clube;
...
public class TorcedorDAO {
    ...
    public List<Torcedor> getAll() {
        ...
        String rawQuery = "SELECT t.*, c.nome FROM " + TorcedorDAO.TABELA_TORCEDOR + " t INNER JOIN " + ClubeDAO.TABELA_CLUBES + " c ON t." + TorcedorDAO.COLUNA_CLUBE_ID + " = c." + ClubeDAO.COLUNA_ID + " ORDER BY " + TorcedorDAO.COLUNA_NOME + " ASC";
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                torcedor.setClube(new Clube(cursor.getInt(1), cursor.getString(3)));
